...
[Gpos_BT, hankel, BTinfo] = BTpos(A,B,C,sig_obs,L_inf,L_pr,r_vals);
% Gpos_BT = Gpos_sp;
...
mupos_ref2 = Gpos_BT(:,:,end)*full_rhs;
...
title('Cross comparison: Posterior covariance error','interpreter','latex','fontsize',16)
savePDF('figs/cross_covs',[5 4],[0 0])

% % plot posterior mean errors
err_LRU = mu_LRU - mupos_ref;
err_LR = mu_LR - mupos_ref;
err_BT = mu_BT - mupos_ref2;


figure(3); clf
semilogy(r_vals,sqrt(sum(err_LR.^2))/norm(mupos_ref)); hold on
semilogy(r_vals,sqrt(sum(err_BT.^2))/norm(mupos_ref2))
...
legend({'Spantini low-rank mean','Balanced truncation','Spantini low-rank update mean','rank of $H$'},'interpreter','latex','fontsize',14,...
    'location','best')
title('Cross comparison: Posterior mean error','interpreter','latex','fontsize',16)
legend boxoff
savePDF('figs/cross_allmeans',[5 4],[0 0])

figure(4); clf
semilogy(hankel,'k'); hold on; semilogy(sqrt(tau2))
legend({'HSVs','$\delta_i$ Spantini'},'interpreter','latex','fontsize',14)
title('Cross comparison: Hankel singular values','interpreter','latex','fontsize',16)
legend boxoff
xlim([0 50])
savePDF('figs/cross_hankel_decay',[5 4],[0 0])
